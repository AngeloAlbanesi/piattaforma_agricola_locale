...
export class PublicAziendeService {
    ...
    getAziende(filters?: PublicAziendaFilters): Observable<PublicAziendeResponse> {
        ...
        return this.http.get<PublicAziendeResponse>(`${this.apiUrl}/azienda/tutteLeAziende`, { params }).pipe(
            map(response => {
                if (response?.content) {
                    response.content.forEach(azienda => this.normalizeAzienda(azienda));
                }
                return response;
            })
        );
    ...
    getAziendaById(id: number): Observable<PublicAziendaDetailDTO> {
        return this.http.get<PublicAziendaDetailDTO>(`${this.apiUrl}/azienda/${id}`).pipe(
            map(azienda => {
                this.normalizeAzienda(azienda);
                return azienda;
            })
        );
    }
    ...
    cercaAziende(query: string, filters?: Omit<PublicAziendaFilters, 'query'>): Observable<PublicAziendeResponse> {
        ...
        return this.http.get<PublicAziendeResponse>(`${this.apiUrl}/azienda/cercaAzienda`, { params }).pipe(
            map(response => {
                if (response?.content) {
                    response.content.forEach(azienda => this.normalizeAzienda(azienda));
                }
    ...
    getCertificazioniAzienda(aziendaId: number): Observable<CertificazioneAziendaDTO[]> {
        return this.http.get<CertificazioneAziendaDTO[]>(`${this.apiUrl}/azienda/${aziendaId}/certificazioni`).pipe(
            map(certificazioni => {
                // Assicura che sia sempre un array
                if (!Array.isArray(certificazioni)) {
                    console.warn('⚠️ [PublicAziendeService] certificazioni non è un array, convertendolo:', certificazioni);
                    if (typeof certificazioni === 'object') {
                        return Object.values(certificazioni);
                    }
                    return [];
                }
    ...
    getProdottiAzienda(aziendaId: number, filters?: { page?: number; size?: number }): Observable<{ content: PublicProdottoSummaryDTO[] }> {
        ...
        return this.http.get<{ content: PublicProdottoSummaryDTO[] }>(`${this.apiUrl}/azienda/${aziendaId}/prodotti`, { params }).pipe(
            map(response => {
                if (response?.content) {
                    response.content.forEach(product => this.normalizeProdotto(product));
                }
                return response;
            })
        );
     ...
     */
    private normalizeAzienda(azienda: any): void {
        if (azienda && azienda.certificazioni) {
            if (!Array.isArray(azienda.certificazioni)) {
                console.warn('⚠️ [PublicAziendeService] certificazioni non è un array, convertendolo:', azienda.certificazioni);
                if (typeof azienda.certificazioni === 'object') {
                    azienda.certificazioni = Object.values(azienda.certificazioni);
                } else {
   ...
   */
    private normalizeProdotto(prodotto: any): void {
        if (prodotto) {
            if (prodotto.certificazioni && !Array.isArray(prodotto.certificazioni)) {
                console.warn('⚠️ [PublicAziendeService] certificazioni prodotto non è un array, convertendolo:', prodotto.certificazioni);
                if (typeof prodotto.certificazioni === 'object') {
                    prodotto.certificazioni = Object.values(prodotto.certificazioni);
                } else {
                    prodotto.certificazioni = [];
                }
            }
            if (prodotto.certificazioniDettagli && !Array.isArray(prodotto.certificazioniDettagli)) {
                console.warn('⚠️ [PublicAziendeService] certificazioniDettagli prodotto non è un array, convertendolo:', prodotto.certificazioniDettagli);
                if (typeof prodotto.certificazioniDettagli === 'object') {
                    prodotto.certificazioniDettagli = Object.values(prodotto.certificazioniDettagli);
                } else {
                    prodotto.certificazioniDettagli = [];
                }
            }
        }
    }

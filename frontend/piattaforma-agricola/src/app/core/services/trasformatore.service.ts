...
export class TrasformatoreService {
    ...
    getMyProcesses(filters?: ProcessoFilters & { pagina?: number, elementiPerPagina?: number }): Observable<PaginatedResponse<ProcessoTrasformazioneSummaryDTO>> {
        ...
        // Aggiungo i parametri di paginazione standard Spring
        ...
        return this.http.get<PaginatedResponse<ProcessoTrasformazioneSummaryDTO>>(`${this.apiUrl}/processi-trasformazione/miei`, { params });
    }
    ...
    // === PRODUTTORI (per selezione fonte materia prima) ===

    /**
     * Ottiene la lista di tutti i produttori disponibili.
     * Usato per selezionare un produttore come fonte di materia prima nelle fasi.
     */
    getAllProduttori(): Observable<ProduttoreSummaryDTO[]> {
        return this.http.get<ProduttoreSummaryDTO[]>(`${this.apiUrl}/utenti/produttori`);
    }
}
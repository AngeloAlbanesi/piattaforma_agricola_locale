...
import { PublicPacchettoDetailDTO, PublicPacchettoSummaryDTO, getPacchettoId, getPacchettoPrezzo } from '../../../../core/models/public.models';
...
export class PacchettoDetailComponent implements OnInit, OnDestroy {
     ...
     */
    navigateToPacchettoObj(pacchetto: PublicPacchettoSummaryDTO): void {
        const id = getPacchettoId(pacchetto);
        this.navigateToPacchetto(id);
    }
    ...
    /**
     * Helper per ottenere il prezzo del pacchetto in modo sicuro
     */
    getPrezzoSicuro(pacchetto: PublicPacchettoSummaryDTO): number {
        return getPacchettoPrezzo(pacchetto);
    }
    ...
    hasSconto(): boolean {
        if (!this.pacchetto?.prezzoScontato) return false;
        const prezzo = getPacchettoPrezzo(this.pacchetto);
        return this.pacchetto.prezzoScontato < prezzo;
    }
    ...
    getSavingsAmount(): number {
        if (!this.hasSconto() || !this.pacchetto) return 0;
        const prezzo = getPacchettoPrezzo(this.pacchetto);
        return prezzo - (this.pacchetto.prezzoScontato || 0);
    }

    getSavingsPercentage(): number {
        if (!this.pacchetto) return 0;
        const prezzo = getPacchettoPrezzo(this.pacchetto);
        return this.calculateSconto(prezzo, this.pacchetto.prezzoScontato || 0);
    }
